...
import { control, elevation, radius, spacing, typography } from '../theme';
...
export function AppFeedbackProvider({ children }: PropsWithChildren) {
  ...
  const showSnackbar = useCallback((message: string, tone: SnackbarTone = 'info') => {
    ...
    setTimeout(() => setSnackbar(current => current?.message === message ? null : current), 2600);
  }, []);
...
export function AppSnackbar({ message, tone }: { message: string; tone: SnackbarTone }) {
  ...
  return <View pointerEvents="none" style={styles.snackbarWrap}><View style={[styles.snackbar, { backgroundColor: colors.surface, borderColor: colors.border }]}><AppIcon name={icon} size={20} color={color} /><Text numberOfLines={3} style={[styles.snackbarText, { color: colors.textPrimary }]}>{message}</Text></View></View>;
}

export function AppConfirmDialog({ visible, title, description, confirmLabel, cancelLabel, danger = false, onConfirm, onCancel }: { visible: boolean; title: string; description?: string; confirmLabel: string; cancelLabel: string; danger?: boolean; onConfirm: () => void; onCancel: () => void }) {
  const { colors } = useAppPreferences();
...
const styles = StyleSheet.create({
  snackbarWrap: { position: 'absolute', left: spacing.lg, right: spacing.lg, bottom: spacing.lg, zIndex: 1000, alignItems: 'center' },
  snackbar: { width: '100%', maxWidth: 680, minHeight: control.buttonHeight, borderWidth: 1, borderRadius: radius.lg, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, ...elevation.floating },
  snackbarText: { ...typography.bodySmall, fontWeight: '700', flex: 1 },
  backdrop: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing.lg },
  dialog: { width: '100%', maxWidth: 520, borderWidth: 1, borderRadius: radius.xl, padding: spacing.lg, ...elevation.floating },
  dialogTitle: { fontSize: 20, lineHeight: 26, fontWeight: '900' },
  dialogDescription: { ...typography.bodySmall, marginTop: spacing.sm },
  dialogActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: spacing.sm, marginTop: spacing.lg, flexWrap: 'wrap' },
  dialogButton: { minHeight: control.buttonHeight, minWidth: layout.minTouchTarget, borderWidth: 1, borderRadius: radius.md, paddingHorizontal: spacing.md, alignItems: 'center', justifyContent: 'center' },
  dialogCancelText: { fontWeight: '800' },
  dialogConfirmText: { fontWeight: '800' },
