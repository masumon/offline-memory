...
export async function readSQLiteBackupData(db: SQLiteDatabase): Promise<SQLiteBackupData> {
  const [appMetadata, tasks, subtasks, memories, notificationDeliveries, versionRow] = await Promise.all([
  ...
  return { appMetadata, tasks, subtasks, memories, notificationDeliveries, schemaVersion: versionRow?.user_version ?? 0 };
}
