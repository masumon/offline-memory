...
export async function pickBackupBundle():Promise<PickedBackup|null>{const result=await DocumentPicker.getDocumentAsync({type:[BACKUP_MIME,ARCHIVE_MIME,'text/json','*/*'],copyToCacheDirectory:true,multiple:false});if(result.canceled)return null;const uri=result.assets[0]?.uri;if(!uri)throw new Error('Selected backup file has no URI');const encoded=await FileSystem.readAsStringAsync(uri,{encoding:FileSystem.EncodingType.Base64});const bytes=base64ToBytes(encoded);if(bytes.length>=2&&bytes[0]===0x50&&bytes[1]===0x4b)return{kind:'archive',bytes};let document:M7BackupDocument;try{document=parseM7BackupDocument(JSON.parse(new TextDecoder().decode(bytes)))}catch(error){throw new Error(error instanceof Error?`Invalid backup file: ${error.message}`:'Invalid backup file')}return{kind:'json',document}}
