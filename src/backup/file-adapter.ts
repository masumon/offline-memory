...
export async function writeBackupArchiveFile(bytes:Uint8Array,createdAt:string):Promise<string>{const filename=`${ARCHIVE_PREFIX}${createdAt.replace(/[:.]/g,'-')}.zip`;const file=new File(Paths.document,filename);file.create({intermediates:true,overwrite:true});file.write(bytes);return file.uri}
...
export async function pickBackupBundle():Promise<PickedBackup|null>{const result=await DocumentPicker.getDocumentAsync({type:[BACKUP_MIME,ARCHIVE_MIME,'text/json','*/*'],copyToCacheDirectory:true,multiple:false});if(result.canceled)return null;const uri=result.assets[0]?.uri;if(!uri)throw new Error('Selected backup file has no URI');const bytes=base64ToBytes(await new File(uri).base64());if(bytes.length>=2&&bytes[0]===0x50&&bytes[1]===0x4b)return{kind:'archive',bytes};let document:M7BackupDocument;try{document=parseM7BackupDocument(JSON.parse(strFromU8(bytes)))}catch(error){throw new Error(error instanceof Error?`Invalid backup file: ${error.message}`:'Invalid backup file')}return{kind:'json',document}}
