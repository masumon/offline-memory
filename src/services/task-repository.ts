...
export async function createTask(db: SQLiteDatabase, input: CreateTaskInput): Promise<Task> {
  ...
  await db.runAsync(
    `INSERT INTO tasks
      (id, title, notes, status, priority, due_at, completed_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)`,
    id,
    title,
    input.notes ?? null,
    ...
    ? await db.getAllAsync<TaskRow>(
        `SELECT id, title, notes, status, priority, due_at, completed_at, created_at, updated_at
         FROM tasks WHERE status = ? ORDER BY
           CASE WHEN due_at IS NULL THEN 1 ELSE 0 END, due_at ASC, created_at DESC LIMIT ?`,
        options.status,
        limit,
      )
    : await db.getAllAsync<TaskRow>(
...
export async function updateTask(
  ...
  await db.runAsync(
    `UPDATE tasks SET title = ?, notes = ?, status = ?, priority = ?, due_at = ?,
