...
function normalizeTags(tags: string[] | undefined): string[] {
  return [...new Set((tags ?? []).map((tag) => normalizeText(tag)).filter(Boolean))].slice(0, 30);
}
...
function rowToMemory(row: Record<string, unknown>): Memory {
  ...
  return {
    id: String(row.id), title: row.title == null ? null : String(row.title), content: String(row.content),
    kind: row.kind as Memory['kind'], source: row.source as Memory['source'], tags,
    importance: Number(row.importance), archived: Number(row.archived) === 1,
...
export async function createMemory(db: SQLiteDatabase, input: CreateMemoryInput): Promise<Memory> {
  ...
  const timestamp = now();
  const id = makeId();
  await db.runAsync(`INSERT INTO memories (id, title, content, kind, source, tags_json, importance, archived, created_at, updated_at, last_accessed_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, NULL)`, id, input.title?.trim() || null, content, input.kind ?? 'NOTE', input.source ?? 'USER', JSON.stringify(normalizeTags(input.tags)), importance, timestamp, timestamp);
...
export async function listMemories(db: SQLiteDatabase, includeArchived = false): Promise<Memory[]> {
  const rows = await db.getAllAsync<Record<string, unknown>>(`${SELECT} WHERE archived = ? ORDER BY importance DESC, updated_at DESC`, includeArchived ? 1 : 0);
...
function scoreMemory(memory: Memory, terms: string[], phrase: string): number {
  ...
  let score = phrase && `${title} ${content}`.includes(phrase) ? 30 : 0;
  for (const term of terms) {
    if (title === term) score += 50;
    else if (title.includes(term)) score += 20;
    if (content.includes(term)) score += 10;
    if (tags.some((tag) => tag === term)) score += 25;
    else if (tags.some((tag) => tag.includes(term))) score += 8;
  }
  return score + Math.max(0, Math.min(5, memory.importance)) * 2 + (memory.lastAccessedAt ? 1 : 0);
}

export async function searchMemories(db: SQLiteDatabase, query: string, matchAll = false): Promise<Memory[]> {
  const normalizedQuery = normalizeText(query);
  const terms = [...new Set(normalizedQuery.split(/\s+/).filter(Boolean))].slice(0, 8);
  if (!terms.length) return listMemories(db);
