...
export const useTaskStore = create<TaskStore>((set) => ({
  ...
  load: async (db) => { set({ isLoading: true, error: null }); try { set({ tasks: await listTasks(db), isLoading: false }); } catch (error) { set({ isLoading: false, error: error instanceof Error ? error.message : 'Unable to load tasks' }); } },
