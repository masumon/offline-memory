...
import { addMemory, archiveStoredMemory, editMemory, findMemories, getActiveMemories, removeMemory, restoreStoredMemory } from '../services/memory-service';
...
export const useMemoryStore = create<MemoryStore>((set) => ({
  ...
  update: async (db, id, input) => { set({ error: null }); try { const memory = await editMemory(db, id, input); if (memory) set((state) => ({ memories: state.memories.map((item) => item.id === id ? memory : item) })); return memory; } catch (error) { set({ error: error instanceof Error ? error.message : 'Unable to update memory' }); return null; } },
  ...
  restore: async (db, id) => { set({ error: null }); try { return restoreStoredMemory(db, id); } catch (error) { set({ error: error instanceof Error ? error.message : 'Unable to restore memory' }); return null; } },
