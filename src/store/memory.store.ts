...
export const useMemoryStore = create<MemoryStore>((set) => ({
  ...
  create: async (db, input) => { set({ error: null }); try { const memory = await addMemory(db, input); set((state) => ({ memories: [memory, ...state.memories] })); if (memory.tags.length > 1) { void import('../services/learning-service').then(m => m.recordTagPairs(db, memory.tags)).catch(() => {}); } return memory; } catch (error) { set({ error: error instanceof Error ? error.message : 'Unable to create memory' }); return null; } },
