...
export function formatBangladeshRelativeDate(value:string|Date,language:AppLanguage,now=new Date()):string{const diff=Math.round((dateKeyToUtcMs(bangladeshDateKey(value))-dateKeyToUtcMs(bangladeshDateKey(now)))/86400000);if(language==='bn'){if(diff===0)return 'আজ';if(diff===1)return 'আগামীকাল';if(diff===-1)return 'গতকাল';}else{if(diff===0)return 'Today';if(diff===1)return 'Tomorrow';if(diff===-1)return 'Yesterday';}return formatBangladeshDate(value,language);}
