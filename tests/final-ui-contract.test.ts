...
describe('final surgical UI contracts',()=>{
 ...
 it('keeps centralized Bangladesh date/time formatters available',()=>{const source=read('src/i18n/date-time.ts');for(const name of ['formatBangladeshDate','formatBangladeshDateTime','formatBangladeshTime','formatBangladeshWeekdayDate','formatBangladeshMonthYear','formatBangladeshRelativeDate','formatBangladeshNumber'])expect(source).toContain(name);});
 it('keeps final shared touch-target primitives at 48dp',()=>{const source=read('src/theme/index.ts');expect(source).toContain('minTouchTarget:48');expect(source).toContain('iconButtonSize:48');});
});
