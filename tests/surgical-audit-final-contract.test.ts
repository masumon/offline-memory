...
describe('surgical audit final contracts',()=>{
 ...
 it('keeps editor attachment panels on persisted entities',()=>{expect(read('app/task-editor.tsx')).toMatch(/<AttachmentPanel\s+ownerType="TASK"\s+ownerId=\{id\}/);expect(read('app/memory-editor.tsx')).toMatch(/<AttachmentPanel\s+ownerType="MEMORY"\s+ownerId=\{id\}/);});
