...
describe('surgical audit final contracts',()=>{
 ...
 it('keeps user-facing errors localized and avoids raw exception rendering in audited editors',()=>{const task=read('app/task-editor.tsx');const memory=read('app/memory-editor.tsx');expect(task).toContain('taskCopy');expect(task).toContain('copy.saveFailed');expect(memory).toContain('memoryCopy');expect(memory).toContain('copy.saveFailed');expect(task).not.toContain('error.message');expect(memory).not.toContain('error.message');});
});
