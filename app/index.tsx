...
export default function HomeScreen() {
  ...
  const handleCreate = async () => {
    ...
    if (task) { setTitle(''); setNlpPreview(null); }
  };
  ...
  return <View style={styles.container}><FlatList data={todayTasks} keyExtractor={(item) => item.id} contentContainerStyle={styles.content}
    ...
    ListFooterComponent={<>
      ...
      {memoriesLoading ? <ActivityIndicator style={styles.loader}/> : recentMemories.length ? recentMemories.map((memory) => <View key={memory.id} style={styles.memoryCard}><Text numberOfLines={2} style={styles.memoryTitle}>{memory.content}</Text></View>) : <Text style={styles.empty}>No memories yet.</Text>}
