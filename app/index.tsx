...
export default function HomeScreen() {
  ...
  return <View style={styles.container}><FlatList data={todayTasks} keyExtractor={(item) => item.id} contentContainerStyle={styles.content}
    ListHeaderComponent={<>
      ...
      <View style={styles.composer}><TextInput value={title} onChangeText={setTitle} onSubmitEditing={() => void handleCreate()} editable={!creating} placeholder="What do you need to do?" placeholderTextColor={colors.textMuted} returnKeyType="done" style={styles.input} accessibilityLabel="Quick capture task"/><Pressable disabled={!title.trim() || creating} accessibilityRole="button" accessibilityLabel="Add task" onPress={() => void handleCreate()} style={[styles.addButton, (!title.trim() || creating) && styles.disabled]}>{creating ? <ActivityIndicator color={colors.onPrimary} /> : <Text style={styles.addButtonText}>Add</Text>}</Pressable></View>
    ...
    renderItem={({ item }) => <TaskRow task={item} completing={completingId === item.id} onComplete={() => void handleComplete(item.id)}/>} ListEmptyComponent={isLoading ? <ActivityIndicator style={styles.loader}/> : <Text style={styles.empty}>No tasks for today. Use Quick Capture above to add one.</Text>}
    ListFooterComponent={<>
