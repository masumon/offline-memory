...
export default function HomeScreen() {
  ...
  const focus = useMemo(() => {
    ...
    const overdue = active.filter((task) => task.dueAt && new Date(task.dueAt).getTime() < today.getTime());
    const dueToday = active.filter((task) => task.dueAt?.slice(0, 10) === todayKey || task.plannedDate === todayKey);
          ...
          <>
            <View style={styles.header}>
              <View style={styles.headerTop}>
                ...
                <Text style={styles.offlineBadge}>● On device</Text>
              </View>
              <Text style={styles.subtitle}>Your tasks and memories stay on this device.</Text>
            </View>
            ...
            <View style={styles.sectionHeader}>
              ...
              <Link href="/planning" asChild>
                <Pressable accessibilityRole="button" accessibilityLabel="Open daily planning" style={styles.textAction}>
                  <Text style={styles.textActionText}>Plan →</Text>
                </Pressable>
              </Link>
            </View>
            <View style={styles.focusGrid}>
              <FocusCard label="Overdue" value={focus.overdue} />
              <FocusCard label="Due today" value={focus.dueToday} />
              <FocusCard label="High priority" value={focus.highPriority} />
            </View>

            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>Today’s tasks</Text>
              <Link href="/planning" asChild>
                <Pressable accessibilityRole="button" accessibilityLabel="View all tasks" style={styles.textAction}>
                  <Text style={styles.textActionText}>View all →</Text>
                </Pressable>
              </Link>
            </View>
          </>
          ...
          <>
            <View style={styles.sectionHeader}>
              ...
              <Link href="/memory" asChild>
                <Pressable accessibilityRole="button" accessibilityLabel="Open memories" style={styles.textAction}>
                  <Text style={styles.textActionText}>View all →</Text>
                </Pressable>
              </Link>
            </View>
            {memoriesLoading ? <ActivityIndicator style={styles.loader} /> : recentMemories.length ? recentMemories.map((memory) => (
...
function TaskRow({ task, onComplete }: { task: Task; onComplete: () => void }) {
  ...
  return <View style={styles.taskRow}>
    <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: completed }} accessibilityLabel={`Complete ${task.title}`} disabled={completed} onPress={onComplete} style={[styles.checkbox, completed && styles.checkboxDone]}>
...
const styles = StyleSheet.create({
  ...
  content: { padding: spacing.xl, paddingTop: spacing.xl, paddingBottom: spacing.xl * 2 },
  ...
  date: { color: colors.textSecondary, fontSize: 15, marginTop: spacing.xs },
  subtitle: { color: colors.textSecondary, fontSize: 14, marginTop: spacing.sm },
  offlineBadge: { color: colors.primary, backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 999, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, fontSize: 12, fontWeight: '700' },
  composer: { flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.lg },
  input: { flex: 1, minHeight: 52, borderWidth: 1, borderColor: colors.border, borderRadius: 14, backgroundColor: colors.surface, color: colors.textPrimary, paddingHorizontal: spacing.md, fontSize: 16 },
  addButton: { minHeight: 52, paddingHorizontal: spacing.lg, borderRadius: 14, backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center' },
  ...
  textAction: { minHeight: 40, justifyContent: 'center', paddingHorizontal: spacing.xs },
  textActionText: { color: colors.primary, fontWeight: '700' },
  focusGrid: { flexDirection: 'row', gap: spacing.sm },
  focusCard: { flex: 1, minHeight: 86, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, padding: spacing.md, justifyContent: 'center' },
