...
export default function SearchScreen() {
  ...
  const labels = bn
    ? { back: 'হোম', eyebrow: 'সার্চ', title: 'সবকিছু খুঁজুন', subtitle: 'এই ডিভাইসে থাকা টাস্ক ও সক্রিয় মেমোরি খুঁজুন।', placeholder: 'টাস্ক ও মেমোরি খুঁজুন', task: 'টাস্ক', tasks: 'টাস্ক', memories: 'মেমোরি', importance: 'গুরুত্ব', empty: 'কোনো মিল পাওয়া যায়নি।', clear: 'সার্চ মুছুন', retry: 'আবার চেষ্টা করুন', searching: 'খোঁজা হচ্ছে…', all: 'সব', filter: 'ফিল্টার' }
    : { back: 'Home', eyebrow: 'SEARCH', title: 'Find anything', subtitle: 'Search tasks and active memories stored on this device.', placeholder: 'Search tasks and memories', task: 'Task', tasks: 'Tasks', memories: 'Memories', importance: 'Importance', empty: 'No matching local data.', clear: 'Clear search', retry: 'Retry', searching: 'Searching…', all: 'All', filter: 'Filter' };
  ...
  return <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.searchBox}><AppIcon name="magnify" size={22} color={colors.textMuted} /><TextInput value={query} onChangeText={setQuery} placeholder={labels.placeholder} placeholderTextColor={colors.textMuted} autoFocus style={styles.input} accessibilityLabel={labels.placeholder} />{query ? <Pressable onPress={() => setQuery('')} accessibilityRole="button" accessibilityLabel={labels.clear} hitSlop={8} style={styles.clearButton}><AppIcon name="close-circle" size={20} color={colors.textMuted} /></Pressable> : null}</View>
    <View accessibilityRole="tablist" style={styles.filters}><Text style={styles.filterLabel}>{labels.filter}</Text>{([['ALL',labels.all],['TASKS',labels.tasks],['MEMORIES',labels.memories]] as const).map(([value,label])=><Pressable key={value} accessibilityRole="tab" accessibilityState={{selected:filter===value}} accessibilityLabel={label} onPress={()=>setFilter(value)} style={({pressed})=>StyleSheet.flatten([styles.filterChip,filter===value&&styles.filterChipSelected,pressed&&styles.pressed])}><Text style={[styles.filterText,filter===value&&styles.filterTextSelected]}>{label}</Text></Pressable>)}</View>
...
function makeStyles(colors: ThemeColors) { return StyleSheet.create({ container: { flex: 1, backgroundColor: colors.background }, content: { width: '100%', maxWidth: layout.contentMaxWidth, alignSelf: 'center', paddingHorizontal: spacing.lg, paddingTop: spacing.md, paddingBottom: spacing.xxl }, header: { paddingTop: spacing.sm, marginBottom: spacing.lg }, back: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: spacing.lg }, backText: { color: colors.primary, fontSize: 16, fontWeight: '800' }, titleRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, titleIcon: { width: 50, height: 50, borderRadius: 16, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' }, titleCopy: { flex: 1, minWidth: 0 }, eyebrow: { color: colors.primary, fontSize: 12, fontWeight: '900', letterSpacing: 1.2 }, title: { color: colors.textPrimary, fontSize: 32, fontWeight: '900', marginTop: 2 }, subtitle: { color: colors.textSecondary, fontSize: 14, lineHeight: 21, marginTop: spacing.xs }, addButton: { minHeight: 48, maxWidth: 100, paddingHorizontal: spacing.sm, borderRadius: 13, backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', gap: spacing.xs }, addText: { color: colors.onPrimary, fontWeight: '800', flexShrink: 1 }, searchBox: { minHeight: 56, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, borderWidth: 1, borderColor: colors.border, borderRadius: 16, backgroundColor: colors.surface, paddingHorizontal: spacing.md, ...elevation.card }, input: { flex: 1, minHeight: 52, color: colors.textPrimary, fontSize: 16 }, clearButton: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center' }, filters: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: spacing.xs, marginTop: spacing.sm }, filterLabel: { color: colors.textMuted, fontSize: 12, fontWeight: '800', marginRight: spacing.xs }, filterChip: { minHeight: 44, borderWidth: 1, borderColor: colors.border, borderRadius: 999, paddingHorizontal: spacing.md, justifyContent: 'center', backgroundColor: colors.surface }, filterChipSelected: { backgroundColor: colors.primary, borderColor: colors.primary }, filterText: { color: colors.textSecondary, fontSize: 12, fontWeight: '800' }, filterTextSelected: { color: colors.onPrimary }, loadingRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.md }, loadingText: { color: colors.textSecondary, fontSize: 13 }, section: { marginTop: spacing.xl }, sectionTitle: { color: colors.textPrimary, fontSize: 19, fontWeight: '900', marginBottom: spacing.sm }, resultCard: { minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 16, padding: spacing.sm, marginBottom: spacing.sm, ...elevation.card }, resultIcon: { width: spacing.lgPlus, height: spacing.lgPlus, borderRadius: 12, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' }, resultCopy: { flex: 1, minWidth: 0 }, resultTitle: { color: colors.textPrimary, fontSize: 14, lineHeight: 20, fontWeight: '700' }, meta: { color: colors.textMuted, fontSize: 11, marginTop: 3 }, pressed: { opacity: 0.78 } }); }
