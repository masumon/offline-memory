...
export default function SearchScreen() {
  ...
  const runSearch = useCallback(
    (value: string, request: number) => {
      setLoading(true);
      setError(null);
      void searchAll(db, value)
        .then((next) => {
          if (request === requestId.current) setResult(next);
        })
        .catch((cause) => {
          if (request === requestId.current) {
            setError(cause instanceof Error ? cause.message : bn ? 'লোকাল ডেটা সার্চ করা যায়নি' : 'Unable to search local data');
          }
        })
        .finally(() => {
  ...
  const labels = bn
    ? {
        ...
        eyebrow: 'সার্চ',
        title: 'সবকিছু খুঁজুন',
        subtitle: 'এই ডিভাইসে থাকা টাস্ক ও সক্রিয় মেমোরি খুঁজুন।',
        placeholder: 'টাস্ক ও মেমোরি খুঁজুন',
        task: 'টাস্ক',
        tasks: 'টাস্ক',
        memories: 'মেমোরি',
        importance: 'গুরুত্ব',
        empty: 'কোনো মিল পাওয়া যায়নি।',
        clear: 'সার্চ মুছুন',
    ...
    : {
        ...
        eyebrow: 'SEARCH',
        title: 'Find anything',
        subtitle: 'Search tasks and active memories stored on this device.',
        placeholder: 'Search tasks and memories',
        task: 'Task',
        tasks: 'Tasks',
        memories: 'Memories',
        importance: 'Importance',
        empty: 'No matching local data.',
        clear: 'Clear search',
        retry: 'Retry',
        searching: 'Searching…',
      };
  ...
  const retry = () => {
    const value = query.trim();
    if (value) runSearch(value, ++requestId.current);
  };
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Link href="/" asChild>
          <Pressable accessibilityRole="button" style={({ pressed }) => [styles.back, pressed && styles.pressed]}>
        ...
        <View style={styles.titleRow}>
          <View style={styles.titleIcon}>
            <AppIcon name="magnify" size={26} color={colors.primary} />
          </View>
          ...
          <Link href="/task-editor" asChild>
            <Pressable accessibilityRole="button" accessibilityLabel={labels.task} style={({ pressed }) => [styles.addButton, pressed && styles.pressed]}>
              <AppIcon name="plus" size={19} color={colors.onPrimary} />
              <Text style={styles.addText}>{labels.task}</Text>
            </Pressable>
          </Link>
        </View>
      </View>
      ...
      <View style={styles.searchBox}>
        ...
        />
        {query ? (
          <Pressable onPress={() => setQuery('')} accessibilityRole="button" accessibilityLabel={labels.clear} hitSlop={8}>
...
function makeStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    back: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: spacing.lg },
    backText: { color: colors.primary, fontSize: 16, fontWeight: '800' },
    titleRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md },
    titleIcon: { width: 50, height: 50, borderRadius: 16, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' },
    titleCopy: { flex: 1, minWidth: 0 },
    eyebrow: { color: colors.primary, fontSize: 12, fontWeight: '900', letterSpacing: 1.2 },
    title: { color: colors.textPrimary, fontSize: 32, fontWeight: '900', marginTop: 2 },
    subtitle: { color: colors.textSecondary, fontSize: 14, lineHeight: 21, marginTop: spacing.xs },
    addButton: { minHeight: 44, maxWidth: 100, paddingHorizontal: spacing.sm, borderRadius: 13, backgroundColor: colors.primary, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', gap: spacing.xs },
    addText: { color: colors.onPrimary, fontWeight: '800', flexShrink: 1 },
    searchBox: { minHeight: 56, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, borderWidth: 1, borderColor: colors.border, borderRadius: 16, backgroundColor: colors.surface, paddingHorizontal: spacing.md, ...elevation.card },
    input: { flex: 1, minHeight: 52, color: colors.textPrimary, fontSize: 16 },
    loadingRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.md },
    loadingText: { color: colors.textSecondary, fontSize: 13 },
    section: { marginTop: spacing.xl },
    sectionTitle: { color: colors.textPrimary, fontSize: 19, fontWeight: '900', marginBottom: spacing.sm },
    resultCard: { minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 16, padding: spacing.sm, marginBottom: spacing.sm, ...elevation.card },
    resultIcon: { width: spacing.lgPlus, height: spacing.lgPlus, borderRadius: 12, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' },
    resultCopy: { flex: 1, minWidth: 0 },
    resultTitle: { color: colors.textPrimary, fontSize: 14, lineHeight: 20, fontWeight: '700' },
